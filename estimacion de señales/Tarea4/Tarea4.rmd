...
# Valores de probabilidad p a evaluar
p_values <- c(0.1, 0.3, 0.5, 0.7, 0.9, 1.0)

# Función para calcular la trayectoria de varianza dado un p
...
p_comparison <- ggplot(df_comparison, aes(x = Tiempo, y = Varianza, color = Probabilidad)) +
  ...
  labs(title = "Análisis de Sensibilidad: Varianza del Error de Filtrado vs Probabilidad p",
       subtitle = "Evolución de P(k/k) para distintas probabilidades de presencia de señal",
       y = "Varianza del Error P(k/k)",
...
**Conclusión**
El análisis confirma que la probabilidad de falsa alarma ($1-p$) actúa como un factor limitante estructural en la precisión del sistema. Incluso con un número infinito de observaciones, la incertidumbre en el mecanismo de observación impide alcanzar el rendimiento del filtro óptimo gaussiano estándar ($p=1$), estabilizándose el error en niveles superiores cuantificables mediante las ecuaciones de Riccati modificadas utilizadas en este estudio.

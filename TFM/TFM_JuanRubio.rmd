...
Consideremos el **Error Cuadrático Medio** (MSE, por sus siglas en inglés) del estimador en un punto $x$ fijo, definido como la esperanza del cuadrado de la diferencia entre el estimador y el valor real:
...
Fenomenológicamente, este equilibrio matemático se manifiesta visualmente en la suavidad de la curva. La elección incorrecta de $h$ conduce a dos patologías opuestas:

* **Infrasuavizado (*Undersmoothing*):** Ocurre cuando $h \ll h_{opt}$. La varianza domina el error. La estimación es ruidosa y presenta falsos picos locales (*spurious modes*) generados por la aleatoriedad de la muestra y no por la estructura de la población.
...
```{r efecto-bandwidth, echo=FALSE, fig.cap="Ilustración del Bias-Variance Tradeoff. (a) Infrasuavizado: Dominio de la varianza. (b) Compromiso adecuado. (c) Sobresuavizado: Dominio del sesgo al cuadrado.", fig.align='center', out.width="100%", fig.height=4, fig.width=10}
